...
function is_user_logged_in($user)
    ...
    if($live_agent_count != '1')
        ...
        if($live_agent_count == '0')
			{
            return _QXZ("Please log in the agent panel as")." $user.";
			} 
		else 
			{
...
$agent_user=$PHP_AUTH_USER;
$qc_user_logged_in=is_user_logged_in($agent_user);
...
<style>
    :root {
        ...
        --call-border: #13515a;
        --call-accent: #00e6bd;
        --call-accent-hover: #42f3d1;
        --call-text: #e8f1f2;
        --call-button-text: #00100d;
    }
    html.qc-light-theme {
        --call-bg: #f1f3f3;
        --call-border: #c4d7d8;
        --call-accent: #007d69;
        --call-accent-hover: #006653;
        --call-text: #15252b;
    ...
    * { box-sizing: border-box; }
    ...
    .qc-call-action:hover { background: var(--call-accent-hover); border-color: var(--call-accent-hover); }
    .qc-call-action.is-secondary { background: transparent; color: var(--call-accent); }
    .qc-call-action.is-secondary:hover { background: var(--call-accent); color: var(--call-button-text); }
    .qc-call-message {
        flex: 1;
        min-width: 0;
        padding: 10px 12px;
    ...
    .qc-call-message.is-error {
        color: #ff8c99;
        border-color: #a83b49;
        background: rgba(168, 59, 73, .12);
    }
    html.qc-light-theme .qc-call-message.is-error {
        color: #9e2635;
        border-color: #d99aa2;
        background: #fff2f4;
    }
...
(function () {
    function applyTheme() {
        var light = false;
        try {
            light = parent.document.documentElement.classList.contains('qc-light-theme');
        } catch (error) {
            light = localStorage.getItem('qc-theme') === 'light';
        }
    ...
    window.callQcLead = async function (event, link) {
        ...
        try {
            ...
            var resultText = String(await response.text()).trim();
            var message = document.createElement('div');
            message.className = 'qc-call-message';
            if (!response.ok || /^ERROR\s*:/i.test(resultText)) message.classList.add('is-error');
            message.textContent = resultText || (response.ok
                ? 'The call request was sent.'
                : 'The call could not be started. Please try again.');
            link.replaceWith(message);
        } catch (error) {
            var message = document.createElement('div');
            message.className = 'qc-call-message is-error';
            message.textContent = 'The call service could not be reached. Please try again.';
            link.replaceWith(message);
        } finally {
...
if ($qc_user_logged_in===true)
	{
    $call_url='qc_api.php?'.http_build_query([
        'source' => 'test',
        'user' => $PHP_AUTH_USER,
        'pass' => $PHP_AUTH_PW,
        'agent_user' => $agent_user,
        'function' => 'external_dial_lead',
        'value' => $phone_number,
        'lead_id' => $lead_id,
    ...
    echo "<a class='qc-call-action' href='".htmlspecialchars($call_url, ENT_QUOTES, 'UTF-8')."' onclick='callQcLead(event, this)'>"._QXZ("Call Lead")."</a>";
	} 
else 
	{
    echo "<div class='qc-call-message'>".htmlspecialchars((string) $qc_user_logged_in, ENT_QUOTES, 'UTF-8')."</div>";
	}

$refresh_url=$PHP_SELF.'?'.http_build_query([
...
echo "<a class='qc-call-action is-secondary' href='".htmlspecialchars($refresh_url, ENT_QUOTES, 'UTF-8')."'>"._QXZ("Refresh")."</a>";
